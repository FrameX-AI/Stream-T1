...
from pipeline import (
    StreamT1CausalInferencePipeline,
)
...
parser.add_argument("--short_threshold", type=float, default=0.0, help="Threshold for short-term reward")
parser.add_argument("--long_threshold", type=float, default=0.2, help="Threshold for long-term reward")
parser.add_argument("--noise_fusion_sigma", type=float, default=-0.1, help="noise_fusion_sigma")
parser.add_argument("--short_score_fusion_threshold", type=float, default=0.4, help="Threshold for short_score_fusion_threshold")
...
pipeline = StreamT1CausalInferencePipeline(config, device=device)
...
config.output_folder = os.path.join(config.output_folder,f"short_{args.short_threshold}+long_{args.long_threshold}+noise_{args.noise_fusion_sigma}+{args.short_score_fusion_threshold}")
pipeline.output_folder = config.output_folder
...
for i, batch_data in tqdm(enumerate(dataloader), disable=(local_rank != 0)):
    ...
    pipeline.inference(
        ...
        noise_fusion_sigma=args.noise_fusion_sigma,
        short_score_fusion_threshold=args.short_score_fusion_threshold
    )
    ...
    try:
      with open(os.path.join(config.output_folder, 'evaluation.json'), "r", encoding="utf-8") as f:
          data = json.load(f)
    except:
        data = {}
    data[os.path.join(config.output_folder,f"{idx}.mp4")] = prompt
    with open(os.path.join(config.output_folder, 'evaluation.json'), "w", encoding="utf-8") as f:
