...
class CausalWanSelfAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.max_attention_size = ( local_attn_size + max_sink_size - sink_size ) * 1560 if max_local != -1 else 32760
        # layers
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            frame_seqlen = math.prod(grid_sizes[0][1:]).item() # H*W
            current_start_frame = current_start // frame_seqlen
            roped_query = causal_rope_apply(
                q, grid_sizes, freqs, start_frame=current_start_frame).type_as(v) # 全局位置编码
            # roped_key = causal_rope_apply(
            #     k, grid_sizes, freqs, start_frame=current_start_frame).type_as(v)

            current_end = current_start + q.shape[1]
            current_sink_size = kv_cache["current_sink_size"].item()
            sink_tokens = current_sink_size * frame_seqlen # 不变token
            ema_sink_tokens = self.ema_sink_size * frame_seqlen
            # If we are using local attention and the current KV cache size is larger than the local attention size, we need to truncate the KV cache
            kv_cache_size = kv_cache["current_kv_cache_size"].item() #窗口大小
            append_sink_start_frames = kv_cache["append_sink_start_frame"].cpu().numpy().tolist()
            ...
            if self.local_attn_size != -1 and (current_end > kv_cache["global_end_index"].item()) and (
                ...
                num_evicted_tokens = num_new_tokens + kv_cache["local_end_index"].item() - kv_cache_size # 丢弃
                num_rolled_tokens = kv_cache["local_end_index"].item() - num_evicted_tokens - sink_tokens # 左移
                ...
                if reward_score is not None:
                    actual_kv_tokens = local_end_index - sink_tokens
                    total_frames = actual_kv_tokens // frame_seqlen
                    global_start_frame =  current_start_frame - total_frames
                    append = reward_score.get(str(global_start_frame), {}).get("append", False)
                    ema = reward_score.get(str(global_start_frame), {}).get("ema", False)
                    # if append and current_sink_size < self.max_sink_size:
                    if append and current_sink_size < self.max_sink_size:
                        append_sink_start_frames.append(global_start_frame)
                else:
                    append = False
                    ema = False
                if append and current_sink_size < self.max_sink_size:
                    # self.current_sink_size += num_new_tokens // frame_seqlen
                    # self.current_kv_cache_size += num_new_tokens

                    current_sink_size += num_new_tokens // frame_seqlen
                    ...
                    # append_k = temp_k[:, sink_tokens:sink_tokens + num_new_tokens].clone()

                    # roped_append_k = causal_rope_apply(
                    #     append_k, grid_sizes, freqs, start_frame=global_start_frame
                    # ).type_as(v)
                    ...
                    cache_update_info = {
                        ...
                        "new_sink_size": current_sink_size,
                        "new_kv_cache_size": kv_cache_size,
                        "new_append_sink_start_frame": global_start_frame,
                ...
                # Compute updated local indices
                    ...
                    evicted_k = temp_k[:, evicted_start:evicted_end].clone()
                    evicted_v = temp_v[:, evicted_start:evicted_end].clone()
                    ...
                    if evicted_tokens_exist and evicted_k is not None and evicted_k.shape[1] > 0 and ( ema or (append and current_sink_size == self.max_sink_size)):
                        # In cases where the number of evicted tokens is less than sink_tokens, we only update the corresponding portion of the sink region
                        
                        # # update the original sink
                        # # min_len = min(evicted_k.shape[1], sink_tokens)
                        # current_sink_k = temp_k[:, :ema_sink_tokens].clone()
                        # current_sink_v = temp_v[:, :ema_sink_tokens].clone()
                        ...
                        # temp_k[:, :ema_sink_tokens] = updated_sink_k
                        # temp_v[:, :ema_sink_tokens] = updated_sink_v

                        # update the last appended sink
                        sink_end_index = sink_tokens
                        sink_start_index = sink_end_index - evicted_k.shape[1]
                        current_sink_k = temp_k[:, sink_start_index:sink_end_index].clone()
                        current_sink_v = temp_v[:, sink_start_index:sink_end_index].clone()
                        ...
                        temp_k[:, sink_start_index:sink_end_index] = updated_sink_k
                        temp_v[:, sink_start_index:sink_end_index] = updated_sink_v

                    # Save cache update info for later use
                    cache_update_info = {
                        "action": "roll_and_insert",
                        "sink_tokens": sink_tokens,
                        "ema_sink_tokens": ema_sink_tokens,
            ...
            if sink_tokens > 0:
                ...
                sink_tokens = current_sink_size * frame_seqlen
                local_budget = self.max_attention_size - sink_tokens
                k_ema_sink = temp_k[:, :ema_sink_tokens]
                roped_k_ema_sink = causal_rope_apply(
                    k_ema_sink, grid_sizes, freqs, start_frame=0
                ).type_as(v)

                if current_sink_size > self.ema_sink_size:
                    roped_k_append_sink_list = []
                    for i in range(len(append_sink_start_frames)):
                        append_sink_start_frame = append_sink_start_frames[i]
                        append_sink_start_tokens = ( i + 1 ) * self.ema_sink_size * frame_seqlen 
                        append_sink_end_tokens = append_sink_start_tokens + self.ema_sink_size * frame_seqlen
                        k_append_sink = temp_k[:, append_sink_start_tokens:append_sink_end_tokens]
                        roped_k_append_sink = causal_rope_apply(
                            k_append_sink, grid_sizes, freqs, start_frame=append_sink_start_frame
                        ).type_as(v)
                        roped_k_append_sink_list.append(roped_k_append_sink)
                    roped_k_append_sinks = torch.cat(roped_k_append_sink_list, dim=1)    
                    roped_k_sink = torch.cat([roped_k_ema_sink, roped_k_append_sinks], dim=1)
                else:
                    roped_k_sink = roped_k_ema_sink
                ...
                if local_budget > 0:
                    ...
                    local_start_for_window = sink_tokens
                    k_local = temp_k[:, local_start_for_window:local_end_index]

                    if local_start_for_window != local_end_index:
                        actual_kv_tokens = local_end_index - local_start_for_window
                        total_frames = actual_kv_tokens // frame_seqlen
                        grid_sizes_kv = copy.deepcopy(grid_sizes)
                        grid_sizes_kv[:, 0] = total_frames
                        global_start_frame =  current_start_frame - total_frames + 3
                        roped_k_local = causal_rope_apply(
                            k_local, grid_sizes_kv, freqs, start_frame=global_start_frame
                        ).type_as(v)
                    else:
                        roped_k_local = k_local

                    v_local = temp_v[:, local_start_for_window:local_end_index]
                    k_cat = torch.cat([roped_k_sink, roped_k_local], dim=1)
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.self_attn = CausalWanSelfAttention(dim, num_heads, local_attn_size, sink_size, max_sink_size, qk_norm, eps)
    ...
    def forward(
        ...
        self_attn_result = self.self_attn(
            ...
            seq_lens, grid_sizes,
            freqs, block_mask, kv_cache, current_start, reward_score, cache_start, sink_recache_after_switch)
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            CausalWanAttentionBlock(cross_attn_type, dim, ffn_dim, num_heads,
                                    local_attn_size, sink_size, max_sink_size, qk_norm, cross_attn_norm, eps)
        ...
        for block_index, (current_end, local_end_index, update_info) in cache_update_infos:
            if update_info is not None:
                ...
                if update_info["action"] == "roll_and_insert":
                    ...
                    ema_sink_tokens = update_info["ema_sink_tokens"]
                    ...
                    sink_start_index = update_info["sink_start_index"]
                    sink_end_index = update_info["sink_end_index"]
                    updated_sink_k = update_info["updated_sink_k"]
                    updated_sink_v = update_info["updated_sink_v"]

                    # Update sink tokens
                    if updated_sink_k is not None and updated_sink_v is not None:
                        cache["k"][:, sink_start_index:sink_end_index] = updated_sink_k
                        cache["v"][:, sink_start_index:sink_end_index] = updated_sink_v    
                ...
                elif update_info["action"] == "append":
                    local_start_index = update_info["local_start_index"]
                    local_end_index = update_info["local_end_index"]
                    write_start_index = update_info.get("write_start_index", local_start_index)
                    write_end_index = update_info.get("write_end_index", local_end_index)
                    new_k = update_info["new_k"]
                    new_v = update_info["new_v"]
                    new_sink_size = update_info["new_sink_size"]
                    new_kv_cache_size = update_info["new_kv_cache_size"]
                    new_append_sink_start_frame = update_info["new_append_sink_start_frame"]
                    # Insert new key/value
                    if write_end_index > write_start_index and new_k.shape[1] == (write_end_index - write_start_index):
                        cache["k"][:, write_start_index:write_end_index] = new_k
                        cache["v"][:, write_start_index:write_end_index] = new_v
                    
                    kv_cache[block_index]["current_sink_size"].fill_(new_sink_size)
                    kv_cache[block_index]["current_kv_cache_size"].fill_(new_kv_cache_size)
                    kv_cache[block_index]["append_sink_start_frame"] = torch.cat([kv_cache[block_index]["append_sink_start_frame"],torch.tensor([new_append_sink_start_frame], dtype=torch.long, device=new_k.device)])
    ...
    def _forward_inference(
        ...
        grid_sizes = torch.stack(
            [torch.tensor(u.shape[2:], dtype=torch.long) for u in x]) # grid_sizes = tensor([[3, 30, 52]])
