...
from wan.modules.causal_model_stream_t1 import CausalWanModel
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                f"wan_models/{model_name}/", local_attn_size=local_attn_size, sink_size=sink_size, max_sink_size=max_sink_size)
            # self.model = CausalWanModel.from_pretrained(
    ...
    def _convert_flow_pred_to_xt(self, flow_pred: torch.Tensor, x0: torch.Tensor, timestep: torch.Tensor) -> torch.Tensor:
        """
        Convert flow matching's prediction to x0 prediction.
        flow_pred: the prediction with shape [B, C, H, W]
        xt: the input noisy data with shape [B, C, H, W]
        ...
        pred = noise - x0
        x_t = (1-sigma_t) * x0 + sigma_t * noise
        we have x_0 = x_t - sigma_t * pred
        see derivations https://chatgpt.com/share/67bf8589-3d04-8008-bc6e-4cf1a24e2d0e
        """
        # use higher precision for calculations
        original_dtype = flow_pred.dtype
        flow_pred, x0, sigmas, timesteps = map(
            lambda x: x.double().to(flow_pred.device), [flow_pred, x0,
                                                        self.scheduler.sigmas,
                                                        self.scheduler.timesteps]
        )
        ...
        timestep_id = torch.argmin(
            (timesteps.unsqueeze(0) - timestep.unsqueeze(1)).abs(), dim=1)
        sigma_t = sigmas[timestep_id].reshape(-1, 1, 1, 1)
        xt_pred = x0 + sigma_t * flow_pred
        return xt_pred.to(original_dtype)
    ...
    def forward(
        ...
        reward_score: Optional[dict] = None,
